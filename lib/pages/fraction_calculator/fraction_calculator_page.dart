// fraction_calculator_page.dart - 第1部分：类定义、初始化和基础显示方法（修复版）
...
class _FractionCalculatorPageState extends State<FractionCalculatorPage>
  ...
  // fraction_calculator_page.dart - 第2部分：特殊显示和分数显示逻辑（修复版）
      ...
      Widget buildFractionDisplay(Fraction fraction) {
        // 【简化】统一负号检查：只检查_integerInput字符串状态
        bool isNegative = calculator.integerInput.startsWith('-');

        // 获取绝对值用于显示（因为负号统一在最左边显示）
        final integerStr = fraction.integerPart.abs().toString();
        final numeratorStr = fraction.numerator.abs().toString();
        ...
        if (onlyNumeratorInput && fraction.denominator == 1) {
          return Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (isNegative) const Text("-", style: TextStyle(fontSize: 47, color: Colors.white, fontWeight: FontWeight.w600)),
              Column(
                mainAxisSize: MainAxisSize.min,
                children: [
          ...
          return Row(
            ...
            children: [
              ...
              Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text("0", style: TextStyle(fontSize: 33, color: Colors.white, fontWeight: FontWeight.w600)),
        ...
        // 特殊情况3：空输入或纯0状态
        if (fraction.integerPart == 0 && fraction.numerator == 0 && !calculator.hasUserInputDenominator) {
        ...
        // 特殊情况4：纯整数显示
        if (fraction.denominator == 1 && fraction.numerator == 0) {
          String displayText = calculator.integerInput.isNotEmpty ? calculator.integerInput : (isNegative ? "-$integerStr" : integerStr);
          return Text(displayText, style: const TextStyle(fontSize: 47, color: Colors.white, fontWeight: FontWeight.w600));
        }

        // 一般情况：完整分数显示（负号统一在最左边）
        return Row(
          mainAxisSize: MainAxisSize.min,
  ...
  // fraction_calculator_page.dart - 第3部分：表达式构建和主界面布局（修复版）
